...
function getTableCalculationFieldMetaInfoValue(queryResponse, fieldName) {
  const queryResponseFieldsMeasures = queryResponse.fields.table_calculations
  f_measure = queryResponseFieldsMeasures.filter(d=>{
    return d.name == fieldName
  })
...
function calculateAggregatedValueTableCalculation(inputArray1D, aggregationType) {
  switch (aggregationType) {
    case "sum":
      return d3.sum(inputArray1D);
      break;
    ...
    case "major-between":
      ...
      break;
    case "latest":
      return inputArray1D[inputArray1D.length - 1];
      break;
  }
}
...
function updateChartBigNumberTrend(data, config, queryResponse) {
  function prepareChartInputParameters(data, queryResponse) {
    ...
    // new stuff for table calculation - start
    firstTableCalculationName = queryResponse.fields.table_calculations[0].name
    console.log(firstTableCalculationName)

    // new stuff for table calculation - end
    ...
    // replace measureName w/ tableCalculationName
    var measureName = firstTableCalculationName
    console.log(measureName)
    measureMetaInfoValue = getTableCalculationFieldMetaInfoValue(queryResponse, measureName)
    console.log(measureMetaInfoValue)
    ...
    // filter w/ non table calculation measure
    dataRecords = tidy(dataRecords, select([xHeaderName, measureName]))
    console.log(dataRecords)
    ...
    var chartTitle = measureMetaInfoValue[0].label;
    var asOfDate = data[0][xHeaderName]["value"];
    console.log(dataRecordsSorted)
    // var asOfDateValue = dataRecordsSorted[0][measureName];
    var asOfDateValue = dataRecordsSorted[dataRecordsSorted.length - 1][measureName];
    console.log(dataRecordsSorted.length - 1)
    console.log(asOfDateValue)
  ...
  metricsValueAggregated = calculateAggregatedValueTableCalculation(dataSeries, config.aggregation_type);
